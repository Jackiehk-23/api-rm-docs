---
id: get-categories-by-store-id
title: Get Categories By Store ID
sidebar_label: Get Categories By Store ID

api:
  method: POST
  url:
    sandbox: https://sb-open.revenuemonster.my/v3/service

  headers:
    Authorization: Bearer {{access_token}}
    X-Signature: sha256 {{signature}}
    X-Nonce-Str: {{nonce}}
    X-Timestamp: {{timestamp}}
    Content-Type: application/json
  body:
    type: json
    example: |
      {
        "service": "alacarte",
        "version": "v1",
        "function": "get.categories",
        "request": {
          "storeId": "{{storeId}}"
        }
      }

examples:
  request: |
    curl --location --request POST 'https://sb-open.revenuemonster.my/v3/service' \
    --header 'Content-Type: application/json' \
    --header 'Authorization: Bearer eyJhbGciOiJSUzI1NiIsImtpZCI6IjIwMTgtMDMtMTMiLCJ0eXAiOiJKV1QifQ..H3G6UDX7sR9EXtTMNs4Q2OHdhUGIhhCfdlAeOpywH4rDuVOcWXXwzF4Imbx8E7I10vFAJpwYZrEkCWCdCCw-WV11y9VT5kP6k75CeS-ZPMOLcKnC5iFT7vEi07r6ovwty9erlcZeXrtrmEIn3rnLva-dxSg3vZ2MyymoNDk-kV7ltXnkoWW4jtXRls6siLhxeY__8kXn2qa0ojVX4Nm6HmzN_vgi-RKSmToMgsdzTF94Y61QVBWhZfolD2-JpHx4qNlklcUdv8HOJ1QHHWpyoJytaJmvr3GJ5G399LbcTLwxB1p2qPg7z4hpoGNu4AP-ybRJVC3P9q9OscQYDNX-dA' \
    --header 'X-Signature: sha256 Sty3LNcKA8+WlMHtAgIY+y1xbwnzKsN0UdyKaW+yYIgcTkBAtF7G5Lx251qQITURJ4wiXPDODxhs1nFVmBBing==' \
    --header 'X-Nonce-Str: VYNknZohxwicZMaWbNdBKUrnrxDtaRhN' \
    --header 'X-Timestamp: 1528450585' \
    --data-raw '{
      "service": "alacarte",
      "version": "v1",
      "function": "get.categories",
      "request": {
          "storeId": "{{storeId}}"
      }
    }'
  body: |
    There is no example body request.
  response: |
    {
      "items": [
        {
          "categoryId": "5f68485ccb3285b9d3955696",
          "storeId": "1556079400120532579",
          "itemIds": ["5f68485ccb3285b9d3955699"],
          "name": "Drinks",
          "status": "ACTIVE",
          "position": 0,
          "createdAt": "2020-09-21T06:29:48.839Z",
          "updatedAt": "2020-09-21T06:29:48.839Z"
        },
        {
          "categoryId": "5f684d01adbcf0b592bc012f",
          "storeId": "1556079400120532579",
          "itemIds": ["5f684d01adbcf0b592bc0129"],
          "name": "Asian Cuisine",
          "status": "ACTIVE",
          "position": 1,
          "createdAt": "2020-04-14T07:17:00.377Z",
          "updatedAt": "2020-09-21T06:49:37.309Z"
        }
      ],
      "cursor": "0",
      "count": 2
    }
---


import ApiEndpoint from "@site/src/components/api/ApiEndpoint";

<ApiEndpoint
  method="POST"
  sandbox="/v3/service"
  prod="/v3/service"
/>




## What is this?

List all menu categories for a specific store.

## When to Use

Use this endpoint when:
- Rendering a menu by category
- Looking up `categoryId` before fetching items

## How to Use

POST `function: "get.categories"` with the `storeId`.

---

### Request Parameters

<ParamTable
  title="Request Parameters"
  rows={[
    { name: "service", type: "String", description: "alacarte service", example: "\"alacarte\"" },
    { name: "version", type: "String", description: "Currently only support 'v1\"", example: "\"v1\"" },
    { name: "function", type: "String", description: "Function that provide by alacarte", example: "\"get.categories\"" },
    { name: "request", type: "Object", description: "Request body",
      children: [
        { name: "storeId", type: "String", description: "ID of the Store that Create from RM", example: "\"1556079400120532579\"" }
      ]
    }
  ]}
/>


### Response Parameters

<ParamTable
  title="Response Parameters"
  rows={[
    { name: "items", type: "Array", description: "List of categories",
      children: [
        { name: "categoryId", type: "String", description: "Category ID", example: "\"5f68485ccb3285b9d3955696\"" },
        { name: "storeId", type: "String", description: "ID of the Store that Create from RM", example: "\"1556079400120532579\"" },
        { name: "itemIds", type: "Array", description: "Category Items", example: "[\"5f68485ccb3285b9d3955699\"]" },
        { name: "name", type: "String", description: "Item Name", example: "\"Drinks\"" },
        { name: "status", type: "String", description: "Status of Category \"ACTIVE\" and \"INACTIVE\"", example: "\"ACTIVE\"" },
        { name: "position", type: "Integer", description: "Current position among Category", example: "0" },
        { name: "createdAt", type: "DateTime", description: "Creation date time", example: "\"2020-09-21T06:29:48.839Z\"" },
        { name: "updatedAt", type: "DateTime", description: "Last update date time", example: "\"2020-09-21T06:29:48.839Z\"" }
      ]
    },
    { name: "cursor", type: "String", description: "Optional, if pagination exists.", example: "\"0\"" },
    { name: "count", type: "Integer", description: "Current page record", example: "2" }
  ]}
/>


<!-- SPDX-License-Identifier: Apache-2.0 -->
